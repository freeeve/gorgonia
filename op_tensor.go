...
func (op repeatOp) UsePreallocDo(prealloc Value, inputs ...Value) (retVal Value, err error) {
	...
	// Fast path: axis-0 repeat with contiguous memory.
	// Rows are contiguous so we can use direct copy() instead of tensor.RepeatReuse.
	if op.along == 0 {
		if dt, ok := t.(*tensor.Dense); ok {
			switch src := dt.Data().(type) {
			case []float32:
				dst := pt.Data().([]float32)
				rowSize := len(src) / dt.Shape()[0]
				dstOff := 0
				for row := 0; row < dt.Shape()[0]; row++ {
					srcRow := src[row*rowSize : (row+1)*rowSize]
					for r := 0; r < rep; r++ {
						copy(dst[dstOff:dstOff+rowSize], srcRow)
						dstOff += rowSize
					}
				}
				return pt, nil
